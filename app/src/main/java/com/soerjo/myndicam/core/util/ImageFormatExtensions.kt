 ...
 * Uses buffer pooling to reduce GC pressure from per-frame allocations
...
fun convertYuvToUyvy(image: Image): ByteArray {
    ...
    val uyvyBuffer = UyvyBufferPool.obtain(width * height * 2)

    return convertYuvToUyvyDirect(
        yBuffer, uBuffer, vBuffer, uyvyBuffer, width, height,
...
/**
 * Optimized YUV to UYVY conversion that reads directly from ByteBuffers
 * and writes to a pre-allocated buffer, reducing allocations significantly
 */
fun convertYuvToUyvyDirect(
    ...
    for (y in 0 until height) {
        for (x in 0 until width step 2) {
            // Get Y values for both pixels (direct from ByteBuffer)
            val yIndex1 = y * yRowStride + x
            val yIndex2 = y * yRowStride + (x + 1)
            val yValue1 = yBuffer.get(yIndex1).toInt() and 0xFF
            val yValue2 = if (x + 1 < width) {
                yBuffer.get(yIndex2).toInt() and 0xFF
            } else {
            ...
            output[uyvyIndex++] = uValue.toByte()     // U
            output[uyvyIndex++] = yValue1.toByte()    // Y0
            output[uyvyIndex++] = vValue.toByte()     // V
            output[uyvyIndex++] = yValue2.toByte()    // Y1
        }
    }
